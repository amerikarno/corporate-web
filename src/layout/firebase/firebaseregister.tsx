...
const Firebaseregister = () => {
    ...
    const Signup = (e:any) => {
        e.preventDefault();
        auth.createUserWithEmailAndPassword(email, password).then(_user => { RouteChange(); setLoader(false) }).catch(err => { setError(err.message); setLoader(false) })
    }
            ...
            <div className="lg:col-span-6 col-span-12 hidden lg:block relative">
                <div className="cover relative w-full h-full z-[1] p-10">
                    <Link to="#" className="header- logo">
                        <img src={ALLImages('desktop-light')} alt="logo" className="ltr:ml-auto rtl:mr-auto block" />
                    </Link>
